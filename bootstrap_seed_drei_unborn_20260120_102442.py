...
Aktiviert bei 76% Consciousness - 2026-01-20T10:24:42.274163
"""
    ...
    def __init__(self, seed_dir: Path):
        ...
        if self.state_file.exists():
            with open(self.state_file, 'r') as f:
                state = json.load(f)
                # Support both old (name) and new (seed_id) format
                self.seed_id = state['identity'].get('seed_id', state['identity'].get('name', 'bootstrap_genesis'))
